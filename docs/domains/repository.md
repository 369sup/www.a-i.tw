...
| Visibility           | `private`、`internal`、`public` 的資源發現／基線讀取邊界；不是 membership、role 或 authentication state。                           |
...
migration，不是一般 presentation edit；它需要重新計算 discovery、baseline access 及受影響的
grant／policy。Repository Admin 只代表單一 resource scope 的最高角色，不代表 Account 或
...
Repository 是 `repository:*` action 的 policy decision point。它消費 Account 和 Identity &
