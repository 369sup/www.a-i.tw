...
The physical portfolio contains 37 descriptors. The following 20 non-planned Contexts are mirrored in
`docs/domains/context-map.json`; the remaining 17 descriptors are governance-only planned Contexts.
...
preserve the 37／20／17 portfolio invariant. Historical test or build results belong in dated evidence files and are
not restated here as current validation.
