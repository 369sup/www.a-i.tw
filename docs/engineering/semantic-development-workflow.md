...
- 目前封閉 portfolio 固定為 37 個 physical Context descriptors：20 個 non-planned runtime Context 與
  17 個 `runtimeEvidence.status: none` 的 planned descriptors。改變數量或 placement 必須先有 superseding
  taxonomy decision，並同步 manifests、migration ledgers、catalogs 與 architecture tests。
