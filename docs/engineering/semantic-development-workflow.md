...
`docs/contracts`、`docs/data`、ADR、Context Map 與 roadmap。Serena memory 只在 formal docs
更新後，依使用者授權寫入精簡導航；memory 不得取代 canonical docs。

### G3 — Approve ownership and path

- Product Context：`apps/web/src/modules/<context>`。
- Internal subdomain：manifest 宣告後位於 `src/subdomains/<name>`。
...
| `.serena`  | 符號影響分析、diagnostics 與經授權的精簡導航 memory      |
