...
建立 Identity & Access、Account 與 Repository 三個 Bounded Context，owner 均為 `www.a-i.tw Product Team`。Identity & Access 發布最小 Principal facts；Account 發布 Account eligibility、Membership 與 Team facts；Repository 透過自身 ACL 消費這些 facts，並獨立擁有 `repository:*` policy、role mapping、visibility、Principal/Team grant 與 lifecycle invariant。
...
Experience 是 inbound adapter，以 Next.js 平行路由提供 Account rail、Repository management 與 Context inspector；concrete in-memory adapters 只在 server composition root 組裝。
...
- production authentication、credential management、enterprise governance、transfer、delete restoration 與 durable outbox 延後。
