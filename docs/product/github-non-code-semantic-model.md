...
狀態：Accepted semantic ownership baseline；runtime status separately evidenced；2026-07-14 官方文件複核
...
| Repository State | Active/archived/disabled product lifecycle state; mutation rules depend on state. | Value Object | Repository | influences authorization and downstream projections | visibility、access decision | R1 | Partial |
...
| Repository Lifecycle | Create、rename、visibility change、archive/unarchive、transfer、delete/restore processes preserving or terminating identity per official rules. | Aggregate behavior / process | Repository | validates target Account and downstream compatibility | permanent Transfer child Entity | R1 | Partial |
...
| Repository disabled/restore exact transition matrix                | Pending verification           | Official lifecycle pages must be reviewed before adding transitions beyond currently evidenced archive behavior.                                             |
