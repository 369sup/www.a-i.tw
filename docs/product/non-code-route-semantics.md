...
GitHub `Issues` 對應目前技術路徑 `apps/web/src/modules/work-management`。該路徑暫時因
Windows／OneDrive 鎖定無法機械改名，但它的正式 Domain 與 Bounded Context 名稱是 `Issues`。
...
6. Search、Entitlement、Audit 與 Enterprise policy 保留各自 GitHub 語意，不合併成 Projection Context。
...
- Agents 與 Copilot。
- Central Context Service、global relationship table 或 Projection Context。
