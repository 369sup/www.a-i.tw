...
GitHub `Issues` 對應 `apps/web/src/modules/collaboration/repository-work/issues`；正式 Domain 與 Bounded Context 名稱
都是 `Issues`。
...
3. Repository owns identity、Account ownership、profile、visibility、state、feature configuration 與 lifecycle；
   Authorization & Policy owns Repository-scoped Role、Grant、Permission 與 access decision。
4. Issues owns Issue、Issue number、Label、Assignment 與 Assignee rules。
5. Subscription owns Watch／conversation interest；Notification owns generated Notification、Inbox state and delivery lifecycle。
