...
Identity & Access、Account、Authorization & Policy 與 Repository 已發布 app-local V1 facts，供已登錄的
...
Current contract flow is Identity & Access／Account facts → Authorization & Policy,
`AuthorizationPolicyApiV1` → Repository, then Repository's compatibility participation facade → Issues／Experience.
