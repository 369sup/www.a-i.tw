...
都有 Domain、Application、Adapter 與 app-facing Public API 的清楚責任；正式產品
...
composition root 綁定 adapter。跨 Context 由 consumer `adapters/outbound/integrations` ACL 經
consumer-owned Port 使用發布方 `contracts/vN/public.ts`。`pnpm arch:check` 將 entrypoints、importer layer、manifest 與
