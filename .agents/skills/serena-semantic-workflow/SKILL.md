...
2. Use `list_memories`, then read only the scoped core/domain/architecture/engineering/status memories required by the task. Treat `docs/` as the formal source of truth: memories are summaries and navigation, never evidence that overrides documentation, code, or tests.
...
7. After semantic edits, obtain diagnostics for changed files and run the repository verification required by `AGENTS.md`. Update formal docs before a concise, source-backed status memory; delete, archive, or promote temporary memories after verification.
