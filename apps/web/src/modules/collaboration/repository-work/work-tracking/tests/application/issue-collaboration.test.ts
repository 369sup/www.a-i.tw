...
import { createIssueCollaborationService } from "../../application/use-cases/issues-collaboration-service";
import { InMemoryIssueCollaborationStore } from "../../adapters/outbound/persistence/in-memory-issue-collaboration";
describe("Issue collaboration", () => {
  ...
  it("comments and records blocking dependencies", async () => {
    ...
    await expect(
      service.comment({
        issueId: "issue-1",
        actor: { principalId: "user-1", status: "active" },
        body: "Update",
      }),
