...
4. MCP 使用原則：repository 檔案與 Git 狀態可使用 filesystem／git MCP 做唯讀調查；對 TypeScript/JavaScript 的符號、實作、引用或 diagnostics，先使用 Serena。若 MCP 未載入，執行 `pnpm mcp:doctor`，並在新的 Codex task 重新載入 `.codex/config.toml`。
